...
All project documentation files (e.g. implementation summaries, integration plans, design docs) must be placed in the `./docs/` directory. The only markdown file that should remain in the repository root is `README.md`.
...
│   ├── DrawnFeaturesPanel.tsx # The drawn-features list: one row per feature
│   │                        #   (inline rename, per-feature style editor,
│   │                        #   measurement / name-label toggles, remove,
│   │                        #   save-to-layers, export). The centre point the
│   │                        #   Circle tool drops with each circle carries a
│   │                        #   'centre' badge instead of the label pencil
...
│   │                        #   read live by the OL geometryFunction) plus the
│   │                        #   centre point it drops beside every circle
...
│   ├── exactPredicates.ts   # Robust orientation and incircle tests with symbolic
│   │                        # perturbation (Simulation of Simplicity style). Used by
│   │                        # the overlay kernel and Delaunay/Voronoi for order-independent,
│   │                        # deterministic results on degenerate inputs (collinear points,
│   │                        # cocircular points). Error-bound based: falls back to symbolic
│   │                        # perturbation only when the determinant is within the float noise
...
│   │                        #   reads the mode live and reports the centre it
│   │                        #   built the ring around, and the per-mode
│   │                        #   auto-name family — including the centre point
│   │                        #   dropped with every circle ('Circle 1 Center')
│   │                        #   and the exact-match counter that keeps those
│   │                        #   points out of the circle sequence. Both modes
│   │                        #   deliver an ordinary 128-vertex Polygon — an
│   │                        #   ol/geom/Circle cannot be written to GeoJSON,
│   │                        #   vertex-edited, measured or fed to the Vector
│   │                        #   Tools, exactly as the rectangle tool already
│   │                        #   converts its Circle sketch via createBox()
...
└── (test files)
    ...
    │                              #   tool switch, circles staying out of the
    │                              #   generic 'Polygon N' counter, and the
    │                              #   centre point dropped with every circle
    │                              #   (exact centre of either flavour, its own
    │                              #   counter-safe name, undo/remove/rename
    │                              #   pairing, session round-trip)
...
The repo root holds one more directory. It is test-only, it verifies code that
lives in `gis_workbench/src/utils/`, and it deliberately sits **outside** the app
package — a Python script, a 92 KB blob of another engine's expectations and the
two suites that read it are not application source:

```
geoprocessing_tool_tests/          # External harness for the Vector Tools panel.
├── README.md                      #   Read this first — it explains the wiring
├── geos-golden.py                 # The GEOS oracle. Asks a real GEOS (the one
│                                  #   QGIS bundles) for validity verdicts,
│                                  #   reasons and locations, ST_MakeValid results,
│                                  #   48 overlay areas and 135 buffer areas. Run
│                                  #   by hand; never at build or test time
├── geosGolden.json                # GENERATED, committed: the answers above, from
│                                  #   GEOS 3.14.1 / shapely 2.0.6. Never edit by
│                                  #   hand — regenerate. Its provenance block
│                                  #   records the generator, GEOS, shapely and
│                                  #   Python versions
├── validity.geos.test.ts          # Check Validity + Make Valid differentially
│                                  #   against GEOS 3.14.1: verdict, reason class,
│                                  #   error location, MakeValid part count + area
└── overlay.geos.test.ts           # The four overlay operators on 12 geometry
                                   #   pairs, and 135 buffers, against GEOS's
                                   #   areas, with the 27 deviations named
```

These are still part of the `gis_workbench` vitest project, not a second one.
Three settings keep them there, all commented in place: `vite.config.ts` widens
`test.include` with `'../geoprocessing_tool_tests/**/*.test.ts'` (vitest discovers
them) and `server.fs.allow` with `'..'` (Vite transforms files outside its root);
`tsconfig.json` includes the folder so `tsc --noEmit` still typechecks it, and
maps the bare specifier `vitest` back to `gis_workbench/node_modules`, because
TypeScript resolves upward from *that* file's directory and there is no
`node_modules` there. So `npm run test:run` and `npm run build` cover them exactly
as they did when they lived in `src/utils/`, and `npm run test:geos` runs just
those two. The engines' own unit and property suites (`overlay.test.ts`,
`overlay.property.test.ts`, `buffer.test.ts`, `geoprocessing.test.ts`,
`geoprocessing.realdata.test.ts`) stayed in `src/utils/` — they have no external
...
- **Utils tests** live alongside their source in `utils/`:
  ...
  - `circleDraw.test.ts` — the Circle tool's two geometries: closed 128-segment rings, a constant *planar* radius for `geometric` vs. a constant *ground* radius for `geodesic` (asserted at 60°N, where the two part company), agreement at the equator, a zero-radius drag, the OL geometryFunction's in-place geometry reuse, live mode getter and centre reporting (a *copy*, from the drawstart call onwards), the per-mode auto-name prefixes, and the centre-point naming + the exact-match counter that keeps 'Circle 1 Center' out of the circle sequence
  ...
  - `validity.geos.test.ts` / `overlay.geos.test.ts` — **differential tests against GEOS 3.14.1**, driven by `geosGolden.json` (generated, committed — see `geos-golden.py`). Both live in **`geoprocessing_tool_tests/`** at the repo root, outside `gis_workbench/`, but run as part of this project's suite (`npm run test:geos` for just these); see §3. Validity: 17 rule-probing cases × verdict, reason class, error location, Make Valid's part count and area, plus "the repair satisfies our own rules". Overlay: 12 geometry pairs × 4 operators, and 135 buffers (13 geometries × 3 distances × cap and join styles, plus 24 single-sided rows at both signs) — 109 of them within 4.2e-13 relative of GEOS, and the 26 the golden file flags with a written reason are asserted to be exactly those rows
...
# Run tests once (CI) — includes ../geoprocessing_tool_tests
npx vitest run

# Run only the differential GEOS suites (../geoprocessing_tool_tests)
npm run test:geos
...
Regenerate the GEOS golden data (from the repo root, only when the case list or a
rule changes — needs a Python with `shapely`; the QGIS one is used because its
GEOS is the GEOS QGIS's own tools run):

```bash
/Applications/QGIS.app/Contents/MacOS/python geoprocessing_tool_tests/geos-golden.py
```

An unchanged case list must regenerate to a **one-line diff or none**. More than
that means the inputs moved, and the reason belongs in the golden file's
`deviation` fields, not in a silently updated expectation.
