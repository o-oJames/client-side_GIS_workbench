...
- **Tile terrain rendering** (XYZ / WMTS / WMS) — tile layers that encode elevation in their RGB channels (AWS *terrarium*, Mapbox *terrain-rgb*, or plain grayscale) can be rendered as **Hillshade** (Horn's 3×3 gradient, QGIS-style sun controls — altitude, azimuth, Z factor, multidirectional blend; runs as an `ol/source/Raster` operation in a worker thread) or **Contours** (marching-squares isolines traced from the decoded elevation grid — the same tracer the COG contours use, with per-level line width, brush style, colour and optional elevation labels, index contours getting a second symbol). The user picks the tile encoding in the raster edit form's *Renderer* section; the raster hides underneath while contours or hillshade are on screen and comes back if tracing fails. Tiles are fetched for the current view extent plus a 10% buffer on each side so contour lines cross tile boundaries seamlessly, decoded from PNG bytes directly (bypassing browser sRGB gamma correction that would corrupt raw elevation data), and composited into a single elevation grid; **the entire heavy pipeline — PNG decode, elevation grid construction, marching-squares tracing for every level, Douglas-Peucker simplification, Chaikin smoothing, and coordinate mapping — runs in a dedicated Web Worker with progressive rendering (contours appear tile-by-tile as each tile is decoded, rather than waiting for all tiles to finish)** so the main thread stays responsive during pan/zoom (it only builds tile URLs, plans contour levels, and creates OpenLayers Features from the coordinate arrays the worker returns); **multi-tile sampling blends elevation values at tile boundaries** (each grid cell is sampled from all tiles overlapping its footprint, not just the tile containing its center) to eliminate the small jumps that terrain tile services introduce when rendering each tile independently; a tile cache avoids re-decoding on every pan. **Dynamic intervals** use five resolution-based tiers (metres per pixel, projection-independent) instead of zoom levels: ≥250 m/px → 500/2500 m, ≥50 → 100/500 m, ≥25 → 50/250 m, ≥5 → 10/50 m, <5 → user settings
...
- **A terrain cross-section window** — right-click any raster layer that is rendering terrain (a COG shown as **Hillshade** or **Contours**, or an XYZ/WMTS tile layer whose RGB channels encode elevation) in the settings panel and choose **Elevation Profile**. The entry only appears while the layer's renderer promises elevations, and its tooltip says which data it will read
- **Pen a line, read the ground** — the window's **Pen** button arms a polyline draw on the map (a dashed line, like the scissors tool's cut line: click to add vertices, double-click or Enter to finish, Esc to abandon). Each finished line is sampled along its length and charted: ground distance on x, elevation on y, with the area under the curve, min/max markers, and a crosshair that also drops a marker on the map at the hovered position — chart and map always agree on an actual sample
- **The same data the renderer shows** — tile layers are read through the terrarium / Mapbox / grayscale decoder the contour and hillshade renderers use (PNG bytes decoded directly, so no sRGB gamma corruption), at the tile zoom the map is at; COGs are read straight off the file's elevation band through the same overview-picking reader the Contours renderer uses. Distances are true ground metres (spherical), never stretched Web Mercator units, and the status bar reports the grid cell size, the tile level or band, the sample count and the chart's vertical exaggeration
- **Honest about nodata** — where the source has no elevation the chart breaks instead of plunging to zero, the readouts report how many samples are missing and the longest gap, and the cumulative ascent/descent counts the step across a gap once (a lower bound) rather than inventing terrain
- **Readouts** — length, min/max elevation with the distance each occurs at, cumulative ascent and descent, steepest gradient, plus a *Detail* selector (120–1200 samples) that re-reads every line, and a *Zero baseline* toggle for the elevation axis
- **Several lines at once** — each drawn line becomes a tab (name, length, relief) with its own remove button; selecting a tab charts that line and highlights it on the map while the others dim
- **Save to the vector layers** — *Save to layer* writes the line as an ordinary GeoJSON LineString feature whose attributes carry the summary numbers (`profile_length_m`, `profile_min_elev_m`, `profile_max_elev_m`, `profile_ascent_m`, `profile_descent_m`, `profile_max_grade_pct`, …) **and every data point behind the chart** in `profile_points` (distance, elevation, lon, lat per sample), so the layer's attribute table shows exactly what was drawn; the saved layer is one click from its table
- **A desktop-OS window** — drag by the title bar, resize from any edge or corner, close top-right; position and size are remembered, and the open layer is persisted per workspace, so the window comes back where you left it (closing it, removing the layer, or switching the layer off its terrain renderer all dismiss it cleanly)
- **Verified end to end** — `utils/elevationProfile.test.ts` covers the plan → grid → points → stats → chart → attributes maths against synthetic grids plus the COG reader against a faked GeoTIFF level; `ElevationProfilePanel.test.tsx` drives the window (pen, draw, tabs, save, gestures) with the terrain read mocked; `SettingsDialog.elevationProfile.test.tsx` covers the menu entry's gating. A browser harness against a synthetic terrarium tile service confirmed the sampled elevations match the encoded DEM to within a quarter of a metre
...
- **One-click theme toggle** in the Settings dialog footer — the pad right of the padlock flips the whole app chrome between the light and the dark palette (moon offers dark, sun offers light; the pad picks up the accent tint while dark mode is on)
- **Follows the OS until you choose** — with no stored preference the app boots into whatever `prefers-color-scheme` says; the first click on the toggle stores an explicit choice that wins from then on (app-wide, not per workspace)
- **Everything repaints from CSS custom properties** — `App.css` keeps every colour it uses in two token blocks (`:root` and `:root[data-theme='dark']`), so the switch is one attribute on `<html>`; no rule anywhere else may hard-code a colour
- **Native controls follow** — `color-scheme` flips with the theme, so scrollbars, checkboxes and colour pickers match; the map's own tiles keep their colours in both themes (a dark basemap preset is still the way to get dark geography)
- The choice survives reloads and travels with the app-lock vault, so unlocking restores the theme you locked with
...
| 23 | **Dark mode / UI theme** | ✅ Done — light/dark chrome toggle in the Settings footer (right of the padlock), persisted app-wide and seeded from the OS `prefers-color-scheme` until the user picks; the whole UI repaints from the `:root` / `:root[data-theme='dark']` token blocks in `App.css`. Basemap tiles keep their own colours in both themes. See [Light & Dark Theme](#light--dark-theme) above. |
...
| 29 | **Elevation profile** | ✅ Done — right-click a terrain-rendered raster layer for the Elevation Profile window: pen a line, read the chart (ground distance × elevation), and save the line with every profile point as its attributes. See [Elevation Profile](#elevation-profile). |
