...
- **Snap to object (AI magic wand)** — the wand tool (4th toolbar button) runs Meta's **SAM 2.1 Tiny** entirely in your browser (ONNX Runtime Web, WebGPU with CPU/WASM fallback): click any object (building, road, paddock…) and its outline is traced into a live polygon preview; **click again to refine**, **Shift+click to exclude** parts ("intelligent scissors"), **Enter** or **double-click** commits the polygon to your drawings, **Esc** cancels. Model sourcing is resilient: a validated copy is cached in **IndexedDB** (nothing re-downloads on refresh); a repaired copy of the model ships with the app under `public/models/sam2.1/` as the offline/CDN-failure fallback; the Hugging Face zip is the last resort
- **Smart snap for lines/polygons (AI)** — right-click the line or polygon tool button to arm smart snap (blue badge): **Alt+click** any object and SAM extracts its mask contour as a dashed background guide; while drawing, **hold Shift** to snap vertices to that contour (vertex + edge snapping, live marker). Right-click the tool again to turn it off
...
| 14 | **Snapping while drawing** | ✅ Partial — AI smart snap: right-click the line/polygon tool, Alt+click an object and SAM 2.1 Tiny extracts its contour; holding Shift then snaps vertices/edges to that contour. Classic snap-to-existing-feature-vertex and snap-to-grid are still missing. |
