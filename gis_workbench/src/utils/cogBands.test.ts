...
import {
  ...
  cogElevationWindow,
  computeCogBandRange,
  expressionHasBooleanArithmetic,
} from './cogBands';
...
describe('hillshade and contour renderers', () => {
  ...
  test('contour parameters fall back to sane intervals, symbols and downscaling', () => {
    const normalised = normalizeCogRender(
    ...
    expect(kept.contour).toEqual({
      ...DEFAULT_CONTOUR,
      interval: 25, indexInterval: 100, color: 'rgba(1,2,3,1)',
    });
  });

  test('contour symbols and downscaling are clamped into drawable ranges', () => {
    const { contour } = normalizeCogRender({
      mode: 'contour',
      contour: {
        lineWidth: 0, indexLineWidth: 99, lineStyle: 'sparkle' as any,
        indexLineStyle: 'dash-dot', inputDownscale: 0, showLabel: false,
      },
    }, floatDemInfo());
    expect(contour).toEqual({
      ...DEFAULT_CONTOUR,
      lineWidth: DEFAULT_CONTOUR.lineWidth,        // 0 is not a stroke width
      indexLineWidth: MAX_CONTOUR_LINE_WIDTH,      // clamped, not rejected
      lineStyle: 'solid',                          // unknown brush style
      indexLineStyle: 'dash-dot',                  // a known one is kept
      inputDownscale: DEFAULT_CONTOUR.inputDownscale,
      showLabel: false,                            // an explicit off survives
    });
    const downscaled = normalizeCogRender(
      { mode: 'contour', contour: { inputDownscale: 999 } }, floatDemInfo(),
    );
    expect(downscaled.contour?.inputDownscale).toBe(MAX_CONTOUR_DOWNSCALE);
  });
...
/**
 * Assert a compiled `color` expression is free of GLSL boolean operators.
 * Anything else in the shader (the `getBandValue` helper, the nodata `discard`)
 * is generated by OpenLayers itself and compiles legally.
 */
function expectCompilable(glsl: string) {
...
describe('OpenLayers WebGL compilation of the generated expressions', () => {
  ...
  test('hillshade compiles: Horn gradient over neighbour pixels', () => {
    ...
    expectCompilable(multi.glsl);
  });

  test('contours have no shader at all — they are traced as vector geometry', () => {
    // A fragment shader cannot give a line a width, a dash pattern or a label,
    // so the Contours renderer leaves the WebGL layer to utils/cogContours.ts
    // and the raster keeps OpenLayers' default mapping as its fallback.
    expect(buildCogColorExpression({ mode: 'contour', band: 1 }, floatDemInfo({ min: 0, max: 100 })))
      .toBeUndefined();
    expect(buildCogRenderStyle({ mode: 'contour', band: 1 }, floatDemInfo({ min: 0, max: 100 })).color)
      .toBeUndefined();
  });
...
// --- shader safety: booleans must stay out of arithmetic --------------------

describe('expressionHasBooleanArithmetic', () => {
  test('every expression the renderers build is boolean-free', () => {
    const dem = floatDemInfo({ min: 0, max: 100 });
    const configs: CogRenderConfig[] = [
      { mode: 'hillshade', band: 1 },
      { mode: 'hillshade', band: 1, hillshade: { multidirectional: true } },
      { mode: 'single', band: 1 },
      { mode: 'rgb', rgb: [1, 2, 3] },
    ];
    for (const config of configs) {
      expect(expressionHasBooleanArithmetic(buildCogColorExpression(config, dem))).toBe(false);
    }
    expect(expressionHasBooleanArithmetic(
      buildCogColorExpression({ mode: 'colormap', band: 1 }, palettedInfo()),
    )).toBe(false);
    expect(expressionHasBooleanArithmetic(
      buildCogColorExpression({ mode: 'rgb', rgb: [4, 3, 2] }, multispectralInfo()),
    )).toBe(false);
  });

  test('a comparison feeding arithmetic is flagged — that is what froze the map', () => {
    const crossing = ['clamp',
      ['+', ['!=', ['band', 1], ['band', 1, 1, 0]], ['!=', ['band', 1], ['band', 1, 0, 1]]],
      0, 1,
    ];
    expect(expressionHasBooleanArithmetic(crossing)).toBe(true);
    expect(expressionHasBooleanArithmetic(['*', ['>', ['band', 1], 0.5], 2])).toBe(true);
    expect(expressionHasBooleanArithmetic(['array', 1, ['==', ['band', 1], 1], 1, 1])).toBe(true);
    expect(expressionHasBooleanArithmetic(['clamp', ['any', ['>', 1, 2], ['<', 3, 4]], 0, 1])).toBe(true);
    // a boolean as the whole colour expression is just as uncompilable
    expect(expressionHasBooleanArithmetic(['any', ['>', 1, 2], ['<', 3, 4]])).toBe(true);
  });

  test('a comparison in a condition slot is legal', () => {
    expect(expressionHasBooleanArithmetic(['case', ['!=', ['band', 1], 0], 1, 0])).toBe(false);
    expect(expressionHasBooleanArithmetic([
      'case', ['>', ['band', 1], 0.5],
      ['array', 1, 0, 0, 1],
      ['array', 0, 0, 0, 0],
    ...
    expect(expressionHasBooleanArithmetic([
      'case', ['any', ['>', ['band', 1], 0.5], ['<', ['band', 1], 0.1]], 1, 0,
    ])).toBe(false);
    expect(expressionHasBooleanArithmetic(['case', ['!', ['>', ['band', 1], 0.5]], 1, 0])).toBe(false);
    // the fallback of a case is a value, not a condition
    expect(expressionHasBooleanArithmetic(['case', ['>', ['band', 1], 0.5], 1, ['band', 1]])).toBe(false);
  });

  test('a number-valued operator in a condition slot is flagged too', () => {
    expect(expressionHasBooleanArithmetic(['case', ['band', 1], 1, 0])).toBe(true);
    expect(expressionHasBooleanArithmetic(['any', ['band', 1], ['>', 1, 2]])).toBe(true);
  });

  test("a palette's colour list is walked without mistaking strings for operators", () => {
    expect(expressionHasBooleanArithmetic(
      ['palette', ['round', ['band', 1]], ['rgba(1,2,3,1)', 'rgba(4,5,6,1)']],
    )).toBe(false);
  });
});
